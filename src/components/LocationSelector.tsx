...
type LocationSelectorProps = {
  ...
  locations: Array<{ name: string; country: string; code: string }>;
  setLocation: React.Dispatch<React.SetStateAction<string>>; // Accepts a setter function
  location: string;
};

const LocationSelector: React.FC<LocationSelectorProps> = ({ placeholderValue, locations, location,setLocation }) => {
      ...
      {isFocused && (
        <div className="absolute top-16 left-0 border bg-white w-full shadow-sm rounded-md z-10">
          {filteredLocations.map((location, index) => (
            ...
            >
              <div>
                <h1 className='text-sm text-gray-800 font-light'>{location.name}</h1>
